...
class NilaiController extends Controller
{
    public function index(){
        ...
        // dd($soals);
        // dd($deletethis->first());
        return view('operators.nilai.index', compact('soals','allusers'));
    }
        ...
        try {
            if(Soal::where([['id',$id],['judul_soal',$judul_soal]])->first()){
                if(User::where([['id',$id_user],['name',$name]])->first()){
                    ...
                    return view('operators.jawaban.detail-jawaban', compact('jawabans'));
                }
                return abort(404);
            }
            return abort(404);
    ...
    public function nilai($id, $judul_soal)
    {
        // try {
            if(Soal::where([['id',$id],['judul_soal',$judul_soal]])->first()){
                ...
                return view('operators.nilai.detail-nilai', compact('allusers','id','judul_soal','answer','id_datasoals'));
            }
            return abort(404);
        // } catch (\Throwable $th) {
        //     // dd($th);
